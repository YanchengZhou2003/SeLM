...
class GPTLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is None:
            ...
            delt = dismatrix_eu - self.distance_matrix_CT
...
def train_model():
    ...
    for iter in range(max_iters):
        ...
        optimizer.step()
    # 保存训练好的模型
    torch.save(model.state_dict(), 'gpt_model3.pth')
    print("Model saved to gpt_model3.pth")

def visualize_similarity():
    # 初始化模型
    model0 = GPTLanguageModel().to(device)
    # 加载训练好的权重
    model0.load_state_dict(torch.load('gpt_model3.pth'))
    model0.eval()
    print("Model loaded from gpt_model3.pth")
    # 获取token嵌入权重
    ...
    # 保存可视化结果
    plt.savefig('token_similarity_heatmap3.png', bbox_inches='tight', dpi=300)
    plt.close()
    print("Similarity matrix visualization saved to token_similarity_heatmap3.png")
