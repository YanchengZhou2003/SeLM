...
class CritiGraph(torch.nn.Module):
    ...
    def loom_TTT(
        self, 
        b_sta     : torch.Tensor,  # (B) 
        b_pos     : torch.Tensor,  # (T)             
        b_val_v   : torch.Tensor,  # (B, T)   , 代表 需要拟合的值
        ...
        splits = list(map(int, splits))
        all_avg_loss = torch.zeros((len(self.devices), B, C, D), dtype=torch.float32, pin_memory=True)
        
        if self.sta_TTT[0] is None:
            self.sta_TTT = [b_sta.to(dev, non_blocking=True) for dev in self.devices]
        ...
        for i, (dev, stream, (s, e)) in enumerate(zip(self.devices, self.streams, zip(splits[:-1], splits[1:]))):
            ...
            with torch.cuda.device(dev), torch.cuda.stream(stream): # type: ignore
                ...
                val_v, val_n = to_dev(
                    b_val_v, b_val_n, 
                    device=dev, s=s, e=e, dim=1 
                )
                pos = to_dev(b_pos, device=dev, s=s, e=e, dim=0)
                ### 通信1：数据传输结束 ###
        ...
        return all_avg_loss.sum(dim=0) # (B, C, tp)
