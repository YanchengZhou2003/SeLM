...
def visualize_similarity():
    ...
    # distance_matrix = torch.exp(torch.mm(token_emb, token_emb.t())).cpu().numpy()  # (vocab_size, vocab_size)
    # distance_matrix = (distance_matrix / torch.norm(distance_matrix, dim=1, keepdim=True)).cpu().numpy()

    # 使用层次聚类对token进行排序
    # 注意：对于大型词汇表，考虑使用method='ward'或减小样本量
    # condensed_dist = pdist(distance_matrix, metric='euclidean')  # 压缩距离矩阵
    # Z = linkage(condensed_dist, method='average')  # 使用平均链接法
    # Z = linkage(condensed_dist, method='ward')  # 使用平均链接法
    # Z = linkage(condensed_dist, method='complete')  # 使用平均链接法
...
if __name__ == "__main__":
    train_model()
    sim_matrix = visualize_similarity()
