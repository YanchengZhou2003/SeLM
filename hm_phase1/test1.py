...
class CritiGraph(torch.nn.Module):
    def __init__(self, h, tp, c, eps, epoch, batch_size, convergence, vocab_size):
        ...
        self.register_buffer('locations', None)  
    ...
    def forward(self, eu_emb):
        ...
        self.locations = torch.randint(0, self.n, (self.vocab_size, self.tp), dtype=torch.int64, device=device)
        dataset = MyDataset(self.li)
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True)

        eu_emb_norm = eu_emb / torch.norm(eu_emb, dim=1, keepdim=True)
        logits = torch.matmul(eu_emb_norm, eu_emb_norm.t()) # (V, V)
        ...
        torch.save(self.state_dict(), 'gpt_model_CT.pth')
...
if __name__ == "__main__":
    ...
    # 获取token嵌入权重
    token_emb = model.token_embedding_table.weight.data  # (vocab_size, n_embd)
    vocab_size = token_emb.size(0)
    model_CT = CritiGraph(h=27, tp=4, c=1, eps=1e-5, epoch=500, batch_size=65, convergence=0.8, vocab_size=vocab_size)
    model_CT(token_emb)
