...
class CritiGraph(torch.nn.Module):
    ...
    @torch.no_grad()
    def loom(self, epoch, batch: Tuple[torch.Tensor, torch.Tensor]): # 注：传入张量在 CPU 的 pinned memory 上
        ...
        stats = [{
            "dev": str(dev),
            "host_t0": None, "host_t1": None,  # 系统时间
            "comm1_ms": 0.0, "comm2_ms": 0.0,  # 两段通信时间（CUDA 事件计）
            "compute_ms": 0.0,                 # 纯计算时间（事件计）
            "sync_t0": None, 'sync_t1': None
        } for dev in self.devices]
        ev_comm1_start: List[torch.cuda.Event] = [None for _ in self.devices] # type: ignore
        ev_comm1_stop: List[torch.cuda.Event]  = [None for _ in self.devices] # type: ignore
        ev_comp_start: List[torch.cuda.Event]  = [None for _ in self.devices] # type: ignore
        ev_comp_stop: List[torch.cuda.Event]   = [None for _ in self.devices] # type: ignore
        ev_comm2_start: List[torch.cuda.Event] = [None for _ in self.devices] # type: ignore
        ev_comm2_stop: List[torch.cuda.Event]  = [None for _ in self.devices] # type: ignore
        ...
        for i, (dev, stream, (s,e)) in enumerate(zip(self.devices, self.streams, zip(splits[:-1], splits[1:]))):
            ...
            with torch.cuda.device(dev), torch.cuda.stream(stream):
                ### 记录系统起始时间 ###
                stats[i]["host_t0"] = time.perf_counter()
                ev_comm1_start[i] = torch.cuda.Event(enable_timing=True)
                ev_comm1_stop[i]  = torch.cuda.Event(enable_timing=True)
                ev_comp_start[i]  = torch.cuda.Event(enable_timing=True)
                ev_comp_stop[i]   = torch.cuda.Event(enable_timing=True)
                ev_comm2_start[i] = torch.cuda.Event(enable_timing=True)
                ev_comm2_stop[i]  = torch.cuda.Event(enable_timing=True)
                
                
                ### 通信1：数据传输开始 ###
                ev_comm1_start[i].record(stream)            
                sta_ind, logits = batch[0][s:e], batch[1][s:e]
                sta_ind, logits = sta_ind.to(dev, non_blocking=True), logits.to(dev, non_blocking=True)
                ev_comm1_stop[i].record(stream)
                ### 通信1：数据传输结束 ###
                
                
                ### 计算：计算开始 ###
                ev_comp_start[i].record(stream)
                ...
                ##### print("sta_loc shape:", sta_loc.shape, sta_loc.dtype, sta_loc.device)
                ...
                logits_eu = logits.unsqueeze(3).unsqueeze(4).expand(-1,-1,-1, cnc_loc.size(2), self.tp) * mask0 # (B, T, T, C, D)
                ...
                ev_comp_stop[i].record(stream)
                ### 计算：计算结束 ###
                
                ### 通信2：数据传输开始 ###
                ev_comm2_start[i].record(stream)
                all_selected_locs[s:e].copy_(selected_locs.to(dtype=torch.int64), non_blocking=True)
                all_loss[i].copy_(tl, non_blocking=True)
                ev_comm2_stop[i].record(stream)
                ### 通信2：数据传输结束 ###
        
        # torch.cuda.synchronize()
        for i, stream in enumerate(self.streams):
            stream.synchronize()
            stats[i]["comm1_ms"]  = ev_comm1_start[i].elapsed_time(ev_comm1_stop[i])
            stats[i]["compute_ms"] = ev_comp_start[i].elapsed_time(ev_comp_stop[i])
            stats[i]["comm2_ms"]  = ev_comm2_start[i].elapsed_time(ev_comm2_stop[i])
            stats[i]["host_t1"] = time.perf_counter()
        
        ### 更新与计算 ###
        
        stats[0]['sync_t0'] = time.perf_counter()
        ...
        stats[0]['sync_t1'] = time.perf_counter()
        
        # 打印每个 stream 的用时
        # host_total = (host_t1 - host_t0) * 1000 毫秒
        for rec in stats:
            if rec["host_t0"] is None:
                continue
            host_ms = (rec["host_t1"] - rec["host_t0"]) * 1000.0
            print(f"[{rec['dev']}] total_host_ms={host_ms:.3f}  comm1_ms={rec['comm1_ms']:.3f}  compute_ms={rec['compute_ms']:.3f}  comm2_ms={rec['comm2_ms']:.3f}")
        
        sync_ms = (stats[0]["sync_t1"] - stats[0]["sync_t0"]) * 1000.0
        print(f"sync_ms: {sync_ms}")
