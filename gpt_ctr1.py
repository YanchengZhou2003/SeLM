...
class GPTLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        logits = ((x * torch.abs(x)).unsqueeze(2) + (token_embeddings * torch.abs(token_embeddings)).unsqueeze(0).unsqueeze(1)).sum(dim=-1) / 2  
        x_int = (x * torch.abs(x) * muloc).to(torch.int32).unsqueeze(2)
        e_int = (token_embeddings * torch.abs(token_embeddings) * muloc).to(torch.int32).unsqueeze(0).unsqueeze(1)
        ...
        logits_ct = (logitso_ct.float() * mulor).sum(dim=-1) / 2
        # logits_ct = self.tree_reduce_bitwise_xor(logitso_ct).float() * mulor / 2
        ...
        if targets is None:
            ...
            gap_weight = 0.000001
            reg = torch.norm(x, dim=2).view(B*T).unsqueeze(1) * torch.norm(token_embeddings, dim=1).unsqueeze(0)
            probs_eu = logits / reg
            probs_ct = (logits_ct / reg).detach()
            # print(probs_ct[0])
            loss_gap = torch.norm(probs_eu - probs_ct, p='fro')
